 ...
 * 短信注册页面
 */
public class ForgetPage extends FakeActivity implements OnClickListener,
        TextWatcher {
    ...
    public void onCreate() {
        int resId = getLayoutRes(activity, "smssdk_forget_page");
        if (resId > 0) {
            ...
            resId = getStringRes(activity, "smssdk_forget");
            if (resId > 0) {
                tv.setText(resId);
            }
            ...
            if (page == 1) {
                ...
                if (res != null && phoneMap != null) {
                    ...
                    if (resId > 0) {
                        ...
                        Toast.makeText(activity, "重置手机号：" + etPhoneNum.getText() + "登陆密码" + etPassword.getText(), Toast.LENGTH_SHORT).show();
