...
public class RegisterPage extends FakeActivity implements OnClickListener,
    ...
    public void onCreate() {
        ...
        if (resId > 0) {
            ...
            if (etPhoneNum.getText().length() > 0 && etPassword.getText().length() >= 8 && etPasswordAgain.getText().length() >= 8) {
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        if (etPhoneNum.getText().length() > 0 && etPasswordAgain.getText().length() >= 8 && etPassword.getText().length() >= 8) {
            btnNext.setEnabled(true);
            ivClear.setVisibility(View.VISIBLE);
            ...
            if (page == 1) {
                ...
                if (res != null && phoneMap != null) {
                    ...
                    if (resId > 0) {
                        ...
                        Toast.makeText(activity, "注册手机号：" + etPhoneNum.getText() + "登陆密码" + etPassword.getText(), Toast.LENGTH_SHORT).show();
